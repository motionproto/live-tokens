...
export function themeFileApi(opts: ThemeFileApiOptions): Plugin {
  ...
  // Themes resource — the documents. Each carries one colors-and-type copy plus
  // a config per off-default component, by value. `_active.json` names the open
  // document, `_production.json` the published one; those two pointers are the
  // only ones the model keeps.
   ...
   * production theme — the one document that says what shipped, read by value.
   * The developer-authored `tokens.css` is never written — it holds defaults the
   ...
   * font stacks) on top. Per-component alias overrides emit only where the
   * production theme carries a config differing from that component's default.
   */
  function regenerateTokensCss(): void {
    ...
    if (fs.existsSync(COMPONENT_CONFIGS_DIR)) {
      ...
      for (const comp of listComponentNames()) {
        const prodCfg = productionTheme?.componentConfigs[comp] as ComponentConfigRead | undefined;
  ...
  /**
   * Regenerate fonts.css from the production theme's embedded fontSources.
   * Called alongside regenerateTokensCss so the production build's static
   * font-loading layer matches what the editor last adopted.
   */
  function syncFontsToCss(colorsAndTypeData: any): void {
  ...
  // artifact: component-configs/{comp}/{default.json,_working.json}. default.json
  // is regenerated from the component's `:global(:root)` block at dev startup and
  ...
  function listLegacyPointers(): string[] {
    ...
    const check = (dir: string) => {
      for (const name of ['_active.json', '_production.json']) {
        ...
        if (fs.existsSync(p)) legacy.push(path.relative(process.cwd(), p));
      }
    };
  ...
  /** A plain rename, unlike the pre-0.48 layout above: nothing inside changes
   *  shape, and a writer aimed at `sketch-styles/` can never destroy a file
   *  under the retired `sketch-presets/` name. Boot only warns, so the rest of
   *  the tree keeps working while a saved sketchstyle waits to be renamed. */
  function warnOnUnmigratedSketchStyles(warn: (msg: string) => void): void {
    ...
    warn(
      `[live-tokens] Saved sketchstyles still live under the retired ` +
        `${path.basename(legacyDir)}/ directory name. Run \`npx live-tokens migrate\` to rename it ` +
        `to ${path.basename(SKETCH_STYLES_DIR)}/; until then they will not show up in the Sketchstyle view.`,
    );
  }
   ...
   *  heals a dangling pointer, so that means someone deleted it mid-session. */
  function activeTheme(): EncapsulatedTheme | null {
  ...
  /** The live colors and type: buffer, else the open theme's copy, else the
   *  shipped default. `_fileName` names the open theme — the document the
   *  payload belongs to — and `_source` says which of the three it came from. */
  ...
  /** The buffer holds one JSON object; anything else is refused with 400.
   *  A stored scalar would make `readWorking`'s `null` ambiguous between
   *  "empty slot" and "stored falsy value". Answers `null` once it has replied. */
  async function readBufferBody(req: any, res: any): Promise<Record<string, unknown> | null> {
    let parsed: unknown;
    try {
  ...
  /** A component the running install does not have. Refusing it keeps a typo'd
   *  URL from creating a config directory for a component that will never
   *  exist. */
  function rejectUnknownComponent(res: any, comp: string): boolean {
  ...
  async function handleComponentConfigByName({ params, req, res }: any) {
    ...
    if (req.method === 'DELETE') {
      ...
      // A named config is a preset; the live state lives in `_working.json` and
      // the saved state in the theme, so nothing has to heal here.
  ...
  /** What the built site ships. Served whole, symmetric with `/themes/active`,
   *  so the client can compare live state against production by content. */
  ...
  async function handleThemeByName({ params, req, res }: any) {
    ...
    if (req.method === 'DELETE') {
      ...
      if (fileName === themesResource.getProductionName()) {
        // The production theme is what the built site ships; deleting it would
        // leave `tokens.generated.css` describing a document nobody can read.
        jsonResponse(res, 403, {
   ...
   *
   * Publishes what is SAVED: buffer edits the client has not written into the
   * theme are invisible from here, which is what makes the bake reproducible.
  ...
  async function handleListSketchStyles({ res }: any) {
  ...
  async function handleSketchStyleByName({ params, req, res }: any) {
    const [fileName] = params;
    if (rejectReservedName(res, fileName)) return;
    const filePath = sketchStylesResource.filePath(fileName);

    if (req.method === 'GET') {
      const raw = sketchStylesResource.readJson(fileName) as any;
      if (!raw) {
        jsonResponse(res, 404, { error: 'Not found' });
        return;
      }
  ...
  //   3. The retired per-layer pointer doors keep 405 shims. Dropping the
