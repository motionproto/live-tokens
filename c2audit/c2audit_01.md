...
- [done] **Resolution (2026-05-04):** Folded into one composable `Scope` primitive in `editorCore.ts` with two orthogonal axes — `collapseToOne` (commit reduces intra-scope mutations to one history entry) and `clipUndoFloor` (active scope clamps undo to its start; commit drops intra-scope past entries). `mutate` is unscoped; `transaction` = scope `{collapseToOne: true, clipUndoFloor: false}`; `paletteSession` = scope `{collapseToOne: true, clipUndoFloor: true}`. `cancelScope({silent})` distinguishes `abortTransaction` (silent: state revert, no tick, no persist — `dirty` reflects pre-tx position) from `cancelPaletteEditSession` (non-silent: full revert + tick + persist). Public API unchanged; all existing tests in `editorStore.test.ts` pass without rewrites.
- [done] **Resolution (2026-05-04, Wave 7b):** Dropped the thin `beginTransaction`/`commitTransaction`/`abortTransaction` and `beginPaletteEditSession`/`commitPaletteEditSession`/`cancelPaletteEditSession` wrappers — they perpetuated the three-regime distinction the fold collapsed. Public API is now `mutate`, `transaction(label, fn)` (sync closure), `beginSliderGesture(label)` (event-wiring), and the explicit handle-based `beginScope({label, collapseToOne, clipUndoFloor}): Scope` / `commitScope(scope)` / `cancelScope(scope, {silent?: false})` trio (with `silent: false` as the default; internal aborts in `undo`/`redo`/competing-scope-collisions explicitly pass `{silent: true}`). Call sites carry the intent in their labels and option object. Inlined at all 10 production call sites (4 in `VariablesTab.svelte`, 6 in `PaletteEditor.svelte`) and all ~40 test call sites (`editorStore.test.ts`, `PaletteEditor.test.ts`). Test count + 18-failure baseline unchanged; `npm run check` 0 new errors.
...
- [done] **Resolution (2026-05-04):** Added `src/lib/storage.ts` with three named helpers — `quietGet` (with optional JSON parse), `quietSet`, and `safeFetch`. Replaced 9 truly-empty try/catch blocks across `columnsOverlay.ts`, `editorPersistence.ts` (the post-Wave-6 home of the old `editorStore.ts:1222/1252` pair), `LiveEditorOverlay.svelte`, `themeInit.ts`, and `ComponentFileManager.svelte`'s `refreshFiles`/`refreshProduction`. The two user-action sites in `ComponentFileManager.svelte` (`handleLoad`, `handleDelete`) wrap multi-step service flows that don't reduce naturally to a single helper call; left intact with stronger explanatory comments (silence is now named, not anonymous).
...
- `src/component-editor/scaffolding/ComponentFileManager.svelte:1` `[local]` `[done]` — 888 lines. Split the SaveAs dialog and the file menu into sub-components; the parent is hard to follow now.
- **Resolution (2026-05-04):** Extracted `SaveAsDialog.svelte` (174 lines: name input, validation, increment helper, confirm flow; emits `on:save` with `{ displayName, fileName }`) and `ComponentFileMenu.svelte` (305 lines: file dropdown + Load dialog with delete buttons; emits `save`/`saveAs`/`openLoad`/`load`/`delete`). Parent shrank from 896 to 551 lines (38% reduction); orchestration logic (persist, refreshFiles, refreshProduction, handleLoad, handleDelete, handleUpdateProduction, handleReset) and the bar's grid CSS stay in the parent. No behavior change — keyboard Cmd/Ctrl+S, click-outside menu close, dirty/applied indicators, status flashes preserved.
