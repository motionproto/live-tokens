...
3. `editorPersistence.ts` — add `normalizeHarmonyAxes(state): EditorState` beside `normalizeComponents`/`migrateGradients` and call it in `hydrate()`'s normalization chain: if `state.harmonyAxes` is a valid array → `sanitizeHarmonyAxes(it, state.palettes)`; otherwise → `axesFromLegacyOrder((state as { harmonyOrder?: unknown }).harmonyOrder, state.palettes)`, and delete the stale `harmonyOrder` key from the merged object. This matters because `hydrate` shallow-merges old persisted localStorage state over `emptyState()` — without it, a user upgrading mid-session would get default axes disconnected from their persisted palettes.
