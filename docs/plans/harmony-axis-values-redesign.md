...
6. **W3 review addendum (fixed in a follow-up commit):** the plan scoped two-way sync to the `paletteBaseColor.ts` setters, but `PaletteEditor.svelte`'s local `edit('baseColor', …)` wrote `baseColor` via raw `mutate`, bypassing `syncBoundAxisHue` — once W3 made `axes[*].hue` load-bearing (mode apply, ghosts, tint anchor), a Palettes-view base-hue edit on a bound family desynced its axis. Fix: `onBaseChange` routes through the shared `setBaseColor` (which syncs), restoring the file-header claim that a Palettes edit is byte-identical to a Colors edit. The `cancelEdit` path needs nothing: `cancelScope` reverts the whole state snapshot, axes included. `loadConfig`/`setPaletteConfig` have no live callers.
