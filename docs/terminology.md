...
The shared vocabulary for the eight bundled skills in `.claude/skills/`. A skill
...
A whole-look request passes through four layers. Each layer takes one noun, and
the nouns stay distinct.

| Layer | Term | What it is | Who writes it |
|---|---|---|---|
| 0 | **the request** | The user's own words, unstructured. | The user |
| 1 | **the design direction** | The requirements derived from the request: the mood, the hue family, the scheme, and the type and geometry that mood implies. Enough to derive the three intents, with the default named where the request leaves a dimension open. | The agent, once |
| 2 | **the color intent**, **the type intent**, **the geometry intent** | What one dimension should achieve, in a line. | The agent, from the design direction and the anchor entry |
| 3 | the base color file, the theme, the pairing, the alias moves | What each executor produces. | The CLI |
...
**The intent is the interface.** A user who invokes set-fonts directly ("make
...
One reading produces all three at once. generate-theme executes the color intent
itself through the base color file and delegates the other two.

**Color is the spine, and it stays inside generate-theme.** The color CLI writes
`themes/<slug>.json`; type and geometry adjust that open theme through unsaved
buffers. Color instantiates the artifact the other two modify, it is the one
step with no skip clause, and the three anchor references fix color, type, and
geometry in one row each. A separate color skill would duplicate or divide those
references and add a hand-off on the one path that always runs.
...
The generator's inputs are now named for what they hold, in prose and on disk:
`scratch/<slug>-base-colors.json` and `scratch/font-pairing.json`.
...
A skill name matches its CLI verb where one exists: live-tokens-generate-theme
runs `generate-theme`, live-tokens-set-fonts runs `set-fonts`,
live-tokens-adjust-geometry runs `adjust-geometry`. Where no verb exists the
skill names its own job, because the CLI validates pages and components and the
agent authors them.
...
`set-fonts` executes the font half of a type intent; the type scale, weights and
