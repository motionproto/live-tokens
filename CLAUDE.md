...
The editor, the dev server, `live-tokens set-colors`, and `live-tokens set-geometry` write into `src/live-tokens/data/` and `src/system/styles/fonts.css`. Loading a theme clears unsaved `_working.json` deltas and sets `themes/_active.json`; live reads fall through to that theme. Adopt sets `themes/_production.json` and rewrites `tokens.generated.css` and `fonts.css`. Generating a theme also creates `themes/<slug>.json`.
