...
## Release script audit

Written in Wave 3 on 2026-09-15 from the scripts at `0cadf4d`. The Approved
column belongs to the user. Wave 3b applies only the rows marked there whose
disposition is other than keep.

- **No script reimplements a rule** in `bin/rules/`. The two likeliest
  overlaps check different facts: `check:component-defaults` compares two files
  that `default-not-token` and `config-token` each read alone, and
  `check:no-style-imports` reads `@import` in `<style>`, which `deep-import`
  reaches only for a deep specifier.
- **`check:overlay-portal` checks a fact every component owes**, and no rule
  carries it to a user project. Its row proposes calling the CLI.
- **`check:production-is-default` shares one condition** with
  `npx live-tokens migrate --check`: both fail on a retired per-layer pointer.

| Script | File | What it checks | Rule module coverage | Proposed disposition | Approved |
|---|---|---|---|---|---|
| `check:no-style-imports` | `scripts/check-no-style-imports.mjs` | Fails when a `.svelte` file anywhere under `src/` holds an `@import` inside a `<style>` block. Under a consumer's `css: 'injected'` the import URL reaches the browser and 404s. | None. `deep-import` in `importsAndRoutes.mjs` reads import specifiers. `check-page` blanks `<style>` first (`codeRegion`). `checkComponentImports` reads a component's whole text and flags an `@import '<spec>'` only when the specifier deep-imports the package. A sibling `@import './x.css'` reaches no rule. | Keep. The fact concerns the `.svelte` files the tarball publishes. | |
| `check:no-tooling-imports` | `scripts/check-no-tooling-imports.mjs` | Fails when a `.ts`, `.js` or `.svelte` file under `src/` imports a path through `../vite-plugin/`, which the tarball omits. CI's `verify.yml` runs it too. | None. `deep-import` in `importsAndRoutes.mjs` covers a user project importing package internals, the opposite direction. | Keep. A user project has no `vite-plugin/` beside its `src/`. | |
| `check:slot-prose` | `scripts/check-slot-prose.mjs` | Fails when a `.svelte` file under `src/system` or `src/demo` holds a `:global()` rule on `p`, `ul`, `ol` or `li` that sets an axis to `inherit`, in a `<style>` block with no `@use` of the `slot-prose` mixin. | None. No rule reads `inherit` pins, and no skill names the mixin. | Keep. The mixin in `src/system/styles/_slot-prose.scss` serves the library's components and demo. | |
| `check:overlay-portal` | `scripts/check-overlay-portal.mjs` | Fails when a `.svelte` file directly in `src/system/components` sets `position: fixed` in a `<style>` block and lacks `use:portal`. The scan reads one directory level. | None. `componentStructure.mjs` has no portal rule, so a user component with a fixed overlay and no portal passes `check-component`. The create-component reference `fixed-overlays.md` tells users `check:overlay-portal` fails the build, and a user project has no such script. | Call the CLI. The fact holds for every component. It first needs a new rule in `componentStructure.mjs` and a public export of `portal` for that rule's guidance to name. No wave schedules either. | |
| `check:editor-font-isolation` | `scripts/check-editor-font-isolation.mjs` | Fails when `src/editor/styles/ui-editor.css`, `ui-form-controls.css`, or a `<style>` block in `Editor.svelte`, `EditorShell.svelte`, `ComponentEditorPage.svelte`, `src/editor/ui/**` or `src/editor/component-editor/**` reads `--font-sans`, `--font-serif`, `--font-display` or `--font-mono`. | None. `raw-text-axis` in `tokens.mjs` flags the same `--font-*` names in page CSS and asks for a text style bundle. `check-page` skips `src/editor` through `NOT_PAGES`. | Keep. Editor chrome ships from the library alone. | |
| `check:component-defaults` | `scripts/sync-component-defaults.mjs --check`, with `scripts/lib/componentBlockSync.mjs` | Fails when a value in a component's first `:global(:root)` block differs from its alias in `component-configs/<id>/default.json`. `sync:component-defaults` writes the same diff. CI's `verify.yml` runs it too. | Each file alone. `default-not-token` in `tokens.mjs` (`checkDefaultsAreSemantic`) requires a token behind each runtime default. `config-token` in `tokens.mjs` (`checkConfigTokens`) requires each `default.json` alias to name a design token or the component's own property. `bin/check-component.test.ts` runs both over the shipped catalogue. Equality between the two files has no rule. | Keep. In a user project the plugin writes `default.json` from the `:global(:root)` block at boot (create-component `SKILL.md` line 217). The library runs the reverse: the editor tunes `default.json`, and this script bakes it into the `.svelte` file. | |
| `check:pages` | `bin/cli.mjs`, running `bin/check-page.mjs` | Runs `check-page --strict --no-fix` over every file `discoverPages` returns: `.svelte` and `.css` under `src/` outside `NOT_PAGES`, plus `main.ts`. Warnings fail, and no file changes. | Full. It runs the CLI, which assembles `componentUse.mjs`, `importsAndRoutes.mjs`, `tokens.mjs` and `testRuns.mjs`. | Keep. It already calls the CLI with `--no-fix`. | |
| `check:production-is-default` | `scripts/check-production-is-default.mjs` | Fails when `themes/_production.json` names a theme other than `default`, a `_working.json` sits in `colors-and-type/`, `themes/` or a `component-configs/<id>/`, a retired per-layer `_active.json` or `_production.json` remains, or `tokens.generated.css` is missing or holds the component-alias override block. `themes/_active.json` is exempt. CI's `verify.yml` runs it too. | None in `bin/rules/`. `npx live-tokens migrate --check` (`runMigrateData` in `bin/migrate.mjs`) exits 1 on a planned data heal, and that plan retires the same per-layer pointers. | Keep. The shipped baseline is a library fact. The pointer condition is a six-line loop, and `migrate --check` also gates tokens.css migrations and data renames. | |
| `check:docs-content` | `scripts/sync-docs.mjs --check` | Fails when `src/editor/docs/content.generated.ts` differs from the module it generates from `src/editor/docs/content/*.md`. | None. | Keep. The docs chapters ship from the library alone. | |
| `check:token-contract` | `scripts/check-token-contract.mjs` | Imports the built engine at `dist-plugin/tokensCssMigrations/index.js`. Fails when an additive migration removes, renames or rewrites a token of `src/system/styles/tokens.css`. When a breaking migration added under `vite-plugin/tokensCssMigrations/migrations/` since the previous `v*` tag ships without a major bump, it warns before 1.0.0 and fails from 1.0.0 on. | None. `vite-plugin/tokensCssMigrations/contract.test.ts` covers the classification half in `npm test`. | Keep. The migrations and their semver gate are the library's release contract. | |
| `check:preset-themes` | `scripts/check-preset-themes.mjs`, with `scripts/lib/presetFonts.mjs` | Needs `dist-plugin/setGeometry/index.js`. For the eight presets in `src/live-tokens/data/themes/`, fails on a missing file, a `schemaVersion` other than 5, a stale `componentSchemaVersion`, a missing component, alias keys that differ from that component's `default.json`, display or body faces that differ from `PRESET_FONTS`, or two presets sharing a card radius and button padding, a display font, or a body font. Across the presets and `default.json`, fails on a `cssVariables` key missing from `tokens.css`. CI's `verify.yml` runs it too. | None. `config-token` in `tokens.mjs` reads the token names inside `default.json` alias values. This script reads key sets, fonts and override keys in theme files, which no rule opens. | Keep. The presets ship from the library alone. | |
| `check:skills` | `scripts/check-skills.mjs`, with `scripts/lib/skillChecks.mjs` and `scripts/lib/cliSurface.mjs` | Per bundled skill, fails on a frontmatter name that differs from its directory, a missing description, more than 250 lines, a code fence over 40 lines or left open, a reference named and absent or present and unnamed, a verb `bin/cli.mjs` does not dispatch, an offered flag of that verb left unnamed, a retired flag or another verb's flag, an unbundled sibling skill, a missing `SAMPLE_PROMPTS` entry, or an "N-step" heading. Across skills, fails when a dispatched verb outside `UNSKILLED_VERBS` reaches no skill, when anchor keys disagree between `design-directions.md` and the three dimension anchor files, and when `token-naming.md` and `KIND_RULES` in `aliasKinds.ts` list different suffixes. | None. | Keep. The bundled skills and the CLI surface are library source. | |
| `check:cli-strings` | `scripts/check-cli-strings.mjs` | Fails when a string literal in `bin/*.mjs`, `bin/lib/*.mjs`, `bin/rules/*.mjs`, `adjustAliases.ts` or `buildColors.ts` holds a retired term (`--family`, theme token, component token, package default, token family) or an off-vocabulary word (ladder, rung, unsaved). | None. It reads the rule modules' own strings, and from Wave 4 on their guidance. | Keep. CLI wording is library source. | |
| `check:skill-sources` | `scripts/sync-skill-sources.mjs --check` | Fails when `src/editor/skill-atlas/skillSources.generated.ts` differs from the module it generates from `.claude/skills/*/SKILL.md` and `references/*.md`. | None. | Keep. The generated module ships in the tarball for the Skill Atlas. | |
| `check:skill-atlas` | `scripts/sync-skill-atlas.mjs` as a dry run, with `scripts/lib/skillAtlas.mjs` | Fails when a tree's digest differs from its `SKILL.md`, a node has no anchor or its anchor moved or vanished, `auditSource` finds a range outside text or a decision label absent from the skill, `auditStructure` finds a malformed tree or restricted vocabulary, or a skill directory has no tree. | None. | Keep. The atlas and the skills it maps are library source. | |
| `check:smoke-install` | `scripts/smoke-install.sh` | Packs the tarball and installs it into a temp project with a plain Svelte Vite config. Resolves `component-editor/contract`, `testing` and `testing/vitest`, asserts the packlist holds `src/testing-js/index.js` and the installed `skill-atlas` export has a non-empty `skillSources.generated.ts` beside it, and runs `vite build` on a page that mounts `<Editor />`. | None. | Keep. The tarball is a library fact. | |
| `check:smoke-create` | `scripts/smoke-create.sh` | Packs the tarball, runs the packed `bin/cli.mjs create`, points the app at the tarball and installs. Asserts `@playwright/test`, `vitest`, `happy-dom` and `live-tokens.testing.ts` are present, asserts postinstall copied every shipped skill and `.gitignore` lists them, then runs `npm run build`. | None. The template's `build` runs `check:design`, which calls `check-page` and `check-component` with `--no-fix`. | Keep. The scaffold and the tarball are library facts. | |
| `check:smoke-component-tests` | `scripts/smoke-component-tests.sh`, with `scripts/lib/componentGate.mjs` | Packs the tarball to a temp directory and builds two projects from the `create` template. Runs `check-component --tests --json` for an authored `beacon` and a shipped `toggle`, across deliberate defects (broken alias, duplicate alias, broken registration, wrong part, wrong registry group, missing test tool, relocated `dataDir`) and a SIGINT. Asserts exit codes, finding rules, coverage, and an unchanged data-directory hash. | None. It runs the CLI from the tarball. | Keep. It proves the tarball's component-testing path. Its CLI calls omit `--no-fix`. | |
| `check:smoke-page-tests` | `scripts/smoke-page-tests.sh`, with `scripts/lib/pageGate.mjs` | Packs the tarball to a temp directory and builds one project from the `create` template. Runs `check-page src/pages/Home.svelte --tests --json` on the clean page, which exits 0 with coverage for all five `page-*` rules at both viewports, and under a `site.css` override, which exits 1 with only `page-component-paint` findings on the page file. Asserts `src/live-tokens/data` keeps its hash across both runs. | None. It runs the CLI from the tarball. | Keep. It proves the tarball's page-testing path. Its CLI calls omit `--no-fix`. | |
