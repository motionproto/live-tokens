...
7. `missing-description` (warn): the runtime file's `catalogue` export is absent, or `description`, `whenToUse`, or `whenNotToUse` in it is missing or malformed, or a `whenNotToUse` row's `use` names no component.
