...
This chapter is for anyone working with the component editor: adding
new components, extending existing ones, or understanding how
linked-block linking, variant groups, and the alias-vs-config split
fit together.

## The registry: single source of truth

`src/editor/component-editor/registry.ts` is the only file that needs
to know about every first-party component. Each entry binds a
canonical lowercase id to its display metadata, source file, editor
component, and token schema:
...
Adding a first-party component means: author the runtime `.svelte`,
author its editor (with `<script module>` exporting `allTokens`), then
add one entry to the registry. Nav-rail order is alphabetical by
`label` (`componentRegistryEntries` is sorted).
...
### Consumer projects: `registerComponent()`

Consumer projects (apps using `@motion-proto/live-tokens` as a
library) do not modify `registry.ts`. They register at runtime via
`registerComponent()` from `src/main.ts`:
...
Consumer-authored components appear under a **CUSTOM** group in the
`/components` nav rail, separated from first-party entries.
`registerComponent` invokes `registerComponentSchema` for you, so you
don't call the latter directly.

See chapter 08 for the full recipe, and the bundled Claude skill
(`live-tokens-add-component`) for an automated version.

### Boot-time validation

`validateRegistryAgainstServerScan(serverIds)` compares the registry's
id list to the filesystem scan returned by
`/api/live-tokens/component-configs`. It logs warnings on
either-direction drift:

- **Registered components missing from server scan.** A registry
  entry whose runtime `.svelte` file is not on disk (or has the wrong
  filename casing).
...
That catches "I added the runtime and the editor, forgot to register
it." Without the validation, the component would silently miss the
nav rail.

## Editor schema: `allTokens`

Each component editor's `<script module>` block exports
`allTokens: Token[]`, the editor's declarative description of its
token surface:
...
That makes `registerComponentSchema` a one-time eager call rather
...
2. For each entry in the post-migration aliases map:
   ...
   - Otherwise if the value starts with `--`, wrap as
...
become their `name`, literals become their `value`); config is
written as-is. The on-disk shape stays stable across the in-memory
restructure, so old files load cleanly without rewrite.

## Sibling linking

A sibling set is a group of tokens that **can** be linked together.
The user toggles whether a set is *currently* linked via the link
icon in each row; the *topology* of what can be linked is
dev-declared.
...
per-component `variable → groupKey` mapping. The schema gets
consulted first when looking up siblings. For unmigrated tokens with
no schema entry, fallback is "split on the last dash" (e.g.
`--button-primary-surface` → group `surface`).

The fallback is documented in `src/system/styles/CONVENTIONS.md`. It
is why the project sometimes uses `-thickness` or `-height` instead
of `-width`: when two unrelated slots both end in `-width` and
neither has a `groupKey`, fallback auto-groups them. The recommended
fix is to declare a `groupKey`; the alternative-property-word
workaround exists for legacy parity.

### The `unlinked` flag (user-toggled)

`ComponentSlice.unlinked` is a list of `groupKey` strings the user
has explicitly broken apart. While a groupKey sits on the unlinked
list, siblings are independently editable; clicking the link icon
again clears it from the list.

`unlinked` lives on the slice (not on the schema) so it persists with
the user's saved config. Unlinking is a property of *this saved
theme of the button*, not of the button's design.

### The linked block

The `<LinkedBlock>` is the row-strip at the top of each editor that
displays one row per linked group, so the user can edit a property
once instead of N times. `computeLinkedBlock(component,
linkableContexts, allTokens, state)`
...
2. If `siblings.length >= 2`, form a group with the rep (representative)
   token plus sibling list plus collected context labels.
...
`TokenLayout` reads back via `getEditorContext().linkedOrder` to
align rows visually.
...
- **`ComponentEditorBase`.** Owns the demo block chrome, the variant
  tab strip (when `tabbable`), the linked block, and the editor
  context (`createEditorContext`).
- **`VariantGroup`.** One per variant, with optional state
  sub-grouping and a "Copy from" menu sourced from `siblings`.
...
- **`TypeEditor`.** Renders typography fieldsets (`TypeGroupConfig`
  rows: colour, family, size, weight, line-height).
...
|---|---|
| `buildTypeGroupTokens(typeGroups)` | The Token entries for colour and four font-shape props per type group, with stable groupKeys (`font-family`, `font-size`, `font-weight`, `line-height`). |
...
codebase already knows how to derive these (see
`StandardButtonsEditor`, `RadioButtonEditor`); extend the helpers if
a new editor needs a variant they do not cover.
...
exclusive. A SegmentedControl option is exactly one of these at a
time. **Disabled is terminal.** A disabled selected button is not a
thing, because once disabled it should look *disabled*, not "the
selected variant's disabled style." Selected-disabled is impossible
by construction.

**Interaction states** are `default`, `hover`. They layer on top of
component states and apply to anything pointer-interactive. A hover
*on* a disabled element is also impossible (the cursor does not get
...
keys parts as states because the editor UI naturally renders one
block per part, but if a state-machine type ever gets introduced,
parts and states must be different types.

If you reintroduce a singular "State" label anywhere, pick the right
noun per `VariantGroup`: "Variants", "Frames", or "Parts", based on
what the group is. The Tabs view sidesteps this because each tab is
named individually.

## Summary

- One registry entry per first-party component: label, icon, source
  file, editor, schema. Consumer projects use `registerComponent()`
  at runtime. The registry is the source of truth for first-party
  components; everything else derives.
- Token schema goes in the editor's `<script module>` as `allTokens`,
  imported by the registry (first-party) or by
  `registerComponent({ schema })` (consumer), registered eagerly.
- Aliases are CSS-var refs (token or literal) consumed by the
  cascade. Config is for JS readers. Do not mix them.
- Sibling linking is dev-declared via `groupKey` plus
  `registerComponentSchema`; users toggle the link, they do not
  define it.
- Disabled is terminal; selected-disabled is impossible. Parts are
  not states.
