/**
 * Components slice — per-component `{ activeFile, aliases, unlinked? }`.
 * Aliases are the full component-token → semantic-token map for whatever
 * config is active; the renderer emits each entry as `var(--<semantic>)`.
 ...
 * stringified aliases bag per component). `componentDirty` re-evaluates on
 * any state change or baseline bump. The baseline is set explicitly by the
 * load path (`setSavedComponentBaseline`) — `loadComponentActive` and
 * `seedComponentsFromApi` in editorStore call into this from their migration
 * pipeline.
 */
