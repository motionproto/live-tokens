...
**isPackage** is one boolean on each `GET /themes` row: the name resolves from the package dir and no distinct local file shadows it. `versionedFileResourceServer.isPackageFile` carries it, and it reads `true` in the library repo, where the local dir IS the package dir, so our own editor lists what a consumer's does. One flag covers both questions the UI asks, because a listed name resolves from somewhere: `isPackage: false` means a local file exists, which is exactly "the user has a copy of this".
