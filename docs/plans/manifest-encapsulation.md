...
A focused review of the themeDirty commit returned PASS: no mutation path escapes the derived store, no write path leaves the baseline stale, and the component-only pins fail under the old gate. Its three LOW findings: the baseline-refresh half now has pins (`themeDirty` false after `persistTheme` and after `loadFromFile`, added with unit 12's close); the shadow-seed and gradient-library items are recorded under Known deferred items, along with its ruling that `commitColorsOnly`'s confirm stays on `$dirty` for now.
...
From the focused review of the themeDirty commit, all pre-existing or latent:

- The gradient library sits outside the theme round trip. `deriveCssVars` folds `gradientsToVars(state.gradients)` into what the page paints, but `themeContent` never serializes `state.gradients` and `themeToState` has no gradient loader, so a theme file's `--gradient-*` strings ride along stale and `makeDefaultGradients()` overrides them at render. Gradient edits never reached disk before the flush fix and still do not; they also do not mark the layer dirty.
- The shadow seed is invisible to history but visible to `themeDirty`. `seedShadowsFromDom` commits via `store.update` with no history entry ("hydration-equivalent"), and the baseline does not absorb it, so a theme file carrying no `--shadow-*` vars would read dirty at rest once the boot seed lands after its load. Unreachable with shipped content: every shipped theme carries all five shadow vars and `generate-theme` carries them forward from the active theme. If ever closed, absorb the seed delta into the baseline; a bare `markThemeSaved` after the seed would erase genuine color dirtiness when the seed fires from the shadows tab over an edited layer.
- `commitColorsOnly`'s discard confirm gates on `$dirty`, so component-only work triggers the warning although a colors-only load preserves component slices and their dirty baselines. The fix is not a swap to `$themeDirty`: the load also resets the gradient library, which only `$dirty` sees (previous item). The right gate is a signal covering theme content plus gradients, so it waits on the gradient item.
